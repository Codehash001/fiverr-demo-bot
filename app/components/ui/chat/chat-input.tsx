...
export default function ChatInput(
  ...
  return (
    <form
      onSubmit={onSubmit}
      className="rounded-xl bg-white p-4 shadow-xl  border space-y-4 shrink-0"
    >
      {imageUrl && (
      ...
      <div className="flex w-full items-start justify-between gap-4 ">
        ...
        <Button type="submit" disabled={props.isLoading || !props.input.trim()} className="hidden md:flex">
          Send message
        </Button>
        <Button type="submit" size={"icon"} disabled={props.isLoading || !props.input.trim()} className="flex md:hidden">
          <Send/>
        </Button>
      </div>
    </form>
